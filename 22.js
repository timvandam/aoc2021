...
const nums = b.trim().split('\n').map(e => e.match(/on|off|-?\d+/g).map((e,i) => isNaN(e) ? e : Number(e)+(i%2==0)))
console.log(nums)

const min = nums.reduce((t, r) => Math.min(t, ...r.slice(1)), Infinity)
const max = nums.reduce((t, r) => Math.max(t, ...r.slice(1)), -Infinity)
const len = max - min + 1

// const mat = Array.from({length:len}, () => Array.from({length:len}, () => Array(len).fill(0)))


function inRange(from, to, num) {
  return num >= from && num <= to
}

function pointInRect([xF, xT, yF, yT, zF, zT], [x, y, z]) {
  return inRange(xF, xT, x) && inRange(yF, yT, y) && inRange(zF, zT, z)
}

/** whether b is inside a */
function rectInRect(a, [bxf, bxt, byf, byt, bzf, bzt]) {
  return pointInRect(a, [bxf, byf, bzf]) && pointInRect(a, [bxt, byt, bzt])
}
...
function* perms(els) {
  ...
  for (const choice of choices) {
    for (const r of perms(rest)) {
      yield [choice, ...r]
    } 
  }
}
function rectPartialInRect(a, [bxf, bxt, byf, byt, bzf, bzt]) {
  const corners = [...perms([[bxf, bxt],[byf, byt],[bzf,bzt]])]
	return corners.some(corner => pointInRect(a, corner))
}

function rectOverlap(a, b) {
  const [axf, axt, ayf, ayt, azf, azt] = a
  const [bxf, bxt, byf, byt, bzf, bzt] = b
  return axf <= bxt && axt >= bxf && ayf <= byt && ayt >= byf && ayf <= byt && ayt >= byf
}

// split a s.t. it does not contain b
function splitRects(a, b) {
  if (rectInRect(b, a)) return [] // a is completely inside b. YEET
  
  // if (!rectPartialInRect(a,b)) return [a] // nothing changes if they dont even touch
  if (!rectOverlap(a,b)) return [a] // nothing changes if they dont even touch
  
  const [axf, axt, ayf, ayt, azf, azt] = a
  const [bxf, bxt, byf, byt, bzf, bzt] = b

  const xSplit = [bxf, bxt].filter(x => inRange(axf, axt, x))
  const ySplit = [byf, byt].filter(y => inRange(ayf, ayt, y))
  const zSplit = [bzf, bzt].filter(z => inRange(azf, azt, z))

  const xBounds = [axf, ...xSplit, axt]
  const yBounds = [ayf, ...ySplit, ayt]
  const zBounds = [azf, ...zSplit, azt]
  
  const rects = []

  for (let x = 0; x < xBounds.length - 1; x++) {
    for (let y = 0; y < yBounds.length - 1; y++) {
      for (let z = 0; z < zBounds.length - 1; z++) {
        const r = [xBounds[x], xBounds[x + 1], yBounds[y], yBounds[y + 1], zBounds[z], zBounds[z + 1]]
        if (!rectInRect(b, r)) rects.push(r)
      }
    }
  }
...
// console.log(splitRects([0, 1, 0, 1, 0, 1], [0.5, 2, 1, 2, 1, 2]))
// xpoints: [[0, 0.5]]
// console.log(splitRects([0, 1, 0, 1, 0, 1], [0.2, 0.3, 0.2, 0.3, 0.2, 0.3]))
// xpoints: [[0, 0.2], [0.3, 1]]
// console.log(splitRects([0, 1, 0, 1, 0, 1], [0.9, 2, 0.9, 2, 1, 2]))
// expected length = 3
// process.exit()

let i = 0;
let rectangles = []
for (let [toggle, xFrom, xTo, yFrom, yTo, zFrom, zTo] of nums) {
  const rect = [xFrom, xTo, yFrom, yTo, zFrom, zTo]
  
  rectangles = rectangles.flatMap(rectangle => splitRects(rectangle, rect)) // for each stored rectangle, remove the current one
  if (toggle==='on') rectangles.push(rect)

  console.log('line',++i)
}
let area = 0
for (const [xF, xT, yF, yT, zF, zT] of rectangles) {
  area += Math.abs(xT - xF) * Math.abs(yT - yF) * Math.abs(zT - zF)
}
console.log('it')
